...
class IPv4:
    ...
    def __init__(self, buffer: bytes):
        b = ''.join(format(byte, '08b') for byte in [*buffer]) # obtain the bitstring
        ...
        self.src = int(b[96:128], 2)
        self.dst = int(b[128:160], 2)
    ...
    for ttl in range(1, 30):
        ...
        for _ in range(PROBE_ATTEMPT_COUNT):
            ...
            if recvsock.recv_select():  # Check if there's a packet to process.
                ...
                # check invalid IP protocol
                ipv4_header = IPv4(buf)
                if ipv4_header.proto != 1:
                    continue
                
                # detect truncated buffer
                if ipv4_header.length > len(buf):
                    continue
                
                buf = buf[ipv4_header.length:] # strip off the IP header

                # check invalid ICMP type
                icmp_header = ICMP(buf)
                if icmp_header.type not in (3, 11):
                    continue

                # check invalid ICMP code
                if icmp_header.type == 11 and icmp_header.code != 0:
                    continue

                if not address[0] in result[-1]:
                    result[-1].append(address[0])
